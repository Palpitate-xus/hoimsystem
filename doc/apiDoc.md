...
`create` 会校验排班存在，且医生、科室、号类必须与排班一致；号源使用条件更新扣减，避免无效排班挂号和并发超卖。挂号记录会保存来源排班，退号时精确返还原时段号源。
...
处方开立只允许 `status=0` 的启用药品；停用或已过期药品会被拒绝，避免失效药品进入收费和发药流程。
...
窗口取消预约优先按预约记录保存的来源排班返还号源；历史无来源排班的记录继续按医生和号类兼容处理。
取消预约采用状态条件更新，重复取消不会重复返还号源。
...
| ✅ | /payment/query/{payment_no} | GET | - | `{ code, msg, data: Payment }`，`status`: 0待支付/1成功/2失败/3已退款 |
...
支付创建时 `amount` 必须为正数且最多保留两位小数，并与收费单金额按分严格一致；少于或多于收费金额 1 分都不会被接受。

`/chargeManagement/refund` 完成收费退款时，会同步关联的成功支付流水为 `status=3`（已退款）。

`/integration/payment/notify` 用于微信/支付宝等支付平台的异步回调。接口不使用患者 JWT，必须通过部署环境变量 `PAYMENT_INTEGRATION_KEY` 配置的 `X-Integration-Key` 鉴权；未配置密钥返回 `503`，密钥错误返回 `401`。服务端会校验回调金额与本地支付单金额一致，金额不一致返回 `400`；外部支付单号重新绑定或本地支付状态已变化返回 `409`。成功回调会原子更新支付单和收费单，重复回调返回 `idempotent: true`。`status` 为 `1` 表示支付成功，`2` 表示支付失败；失败回调的 `failure_reason` 会保存到支付流水，便于对账和排查。
