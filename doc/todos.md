# 医院信息管理系统（HIS-OP）待办事项

> 本文件记录系统所有功能模块的开发状态。状态标记：✅ 已完成 | 🚧 进行中 | ⬜ 待开发

---

## 〇、业务领域开发状态总览

按业务领域分组的全景视图，便于快速定位模块。

### 1. 门诊管理 ✅
- 患者服务：智能导诊、预约挂号、现场挂号、缴费、病历/处方查询、健康档案、评价、预交金、转诊
- 医生工作站：排班、病历、处方、检查检验申请、考勤、MDT会诊、临床路径
- 药房管理：药品/耗材管理、发药、库存预警/盘点、处方点评、采购、ADR监测
- 收费管理：费用管理、发票、窗口挂号、日结对账
- 排队叫号：分诊台、候诊队列、候诊巡视
- 报到签到：预约报到、违约记录
- 护士预检、检验科结果录入、复诊随访

### 2. 住院管理 ✅
- 病区床位、入院登记、住院医嘱、护士工作站、住院费用、出院结算
- 手术麻醉（申请/排程/麻醉记录）
- 电子病历入口

### 3. 电子病历(EMR) ✅
- 病历模板、结构化病历、病程记录、查房记录、病历质控

### 4. 体检系统 ✅
- 体检套餐、项目、预约、记录、结果、报告

### 5. 系统管理 ✅
- 用户权限、操作日志、数据字典、系统参数、消息中心、数据备份、不良事件上报、CA数字签名

### 6. 病案管理 ✅
- 病案首页（ICD-10编码）、归档、借阅、封存

### 7. 急诊管理 ✅
- 急诊分诊（4级）、抢救记录、留观管理、绿色通道、急诊病历

### 8. 护理文书 ✅
- 入院护理评估、护理计划、交接班记录、危重护理记录

### 9. 门诊输液 ✅
- 输液医嘱、配药核对、输液执行、皮试管理

### 10. PACS/RIS 基础闭环 ✅
- 检查申请、影像报告书写/审核、外部 Viewer、PACS 幂等回调和持久化出站事件；真实 DICOM 存储与厂商协议仍需现场联调

### 11. 检验科 LIS 扩展 ✅
- 检验标本管理、LIS 幂等回调、检验审核、危急值通知/确认/处理闭环和持久化出站事件

### 12. 抗菌药物管理 ✅
- 分级目录、越级审批、DDDs统计、送检率统计

### 13. 院感管理 ✅
- 院感监测、暴发预警、消毒监测、职业暴露、院感上报

### 14. 物资设备管理 ✅
- 设备台账、维修、保养、高值耗材追溯、资产盘点

### 15. 血库管理 ✅
- 用血申请、血型复核、交叉配血、发血、输血反应

### 16. 医保管理基础闭环 ✅
- 目录对照、医保结算/回调、慢病管理、版本化 DRG/DIP 自动分组、医保控费和出站事件；国家/地方平台签名、电子凭证需现场适配

---

## 一、已完成

- [x] 数据库关系模型建模（15 张核心表）
- [x] 需求文档编写
- [x] API 文档编写（82 个接口，按 13 个模块分类）
- [x] 数据库文档编写（25 张表，含规划表）
- [x] FastAPI 后端基础框架搭建（当前主后端）
- [x] Vue 3 前端框架搭建（vue3-new-ui，当前主前端）
- [x] 登录认证模块（登录/注册/公钥/用户信息/登出）
- [x] 管理员模块（医生查询、病人查询、科室增查、通知公告增查）
- [x] 病人模块（预约挂号、现场挂号、挂号/预约记录查询、缴费）
- [x] 挂号序号按自然日递增，患者端和窗口端返回并展示本次序号
- [x] 医生模块（医生注册、排班管理、处方开具/查询）
- [x] 药品管理模块（药品入库、查询、库存查询）
- [x] vue3-new-ui 前端页面联调（主分支）
- [x] **住院管理模块**（病区床位/入院登记/医嘱/护理/费用/出院）
- [x] **手术麻醉模块**（手术申请/排程/麻醉记录）
- [x] **结构化电子病历模块**（模板/病程记录/查房/质控）
- [x] **体检系统模块**（套餐/项目/预约/记录/报告）

> **已弃用保留项**：vue-ui（Vue 2.x 前端）、django_be（Django 后端）

---

## 二、后端接口建设记录

### 2.1 高优先级

- [x] 病历管理 API（书写/查询/详情/修改）
- [x] 排队叫号 API（队列查询/叫号/过号/跳号）
- [x] 医生信息修改/删除 API
- [x] 病人信息修改 API
- [x] 科室信息修改/删除 API
- [x] 通知公告修改/删除 API
- [x] 药品信息修改/删除 API
- [x] 处方取消 API

### 2.2 中优先级

- [x] 报到签到 API
- [x] 护士预检 API（生命体征录入/查询）
- [x] 检查检验申请 API
- [x] 检查检验结果录入/查询 API
- [x] 药房处方审核/发药/退药 API
- [x] 退费处理 API
- [x] 发票管理 API（生成/查询/打印）
- [x] 医生考勤签到 API

### 2.3 低优先级

- [x] 复诊预约 API
- [x] 随访管理 API（计划/记录）
- [x] 满意度评价 API
- [x] 智能导诊 API
- [x] 报表统计 API（门诊量/财务/药品/工作量）

### 2.4 病人端扩展 API（就诊流程闭环）

> **场景**：病人挂号后实时查看排队进度、接收消息提醒、管理家庭成员、下载电子票据。

- [x] 排队进度查询 API（当前前面还有几人、预计等待时间）
- [x] 消息通知中心 API（获取/标记已读/删除）
- [x] 家庭成员管理 API（增删查改，为家人挂号）
- [x] 电子票据下载 API（PDF 发票）
- [x] 就诊导航/科室位置 API（科室位置、电话和医生信息）

### 2.5 医生端扩展 API（诊疗深度）

> **场景**：医生接诊时查看预检信息、检验结果，支持转诊/会诊，使用模板快速开方。

- [x] 转诊申请 API（转科/转院，含原因、目标科室、接收医生）
- [x] 会诊申请 API（邀请多科室医生会诊）
- [x] 医嘱管理 API（长期/临时医嘱开具/停止/查询/撤销） *（住院医嘱已实现）*
- [x] 处方模板 API（创建/查询/应用/删除个人常用处方）
- [x] 诊断模板 API（ICD-10 编码/常用诊断模板维护）
- [x] 今日候诊病人列表 API（按医生过滤的候诊队列，含预检信息）
- [x] 病人检验结果查看 API（医生查看自己病人的检验报告）
- [x] 停诊/加号申请 API（医生申请调休或额外加号，需主任审批）

### 2.6 护士端扩展 API（护理闭环）

> **场景**：护士完成预检后，执行医嘱（输液/注射/皮试），记录护理文书。

- [x] 输液管理 API（开具/执行/巡视/结束，含滴速、药物批次）
- [x] 注射管理 API（肌注/皮下/皮内注射记录）
- [x] 皮试管理 API（皮试药物、观察时间、结果判定）
- [x] 护理记录 API（创建/查询，含病情观察、护理措施、效果评价） *（住院护理记录已实现）*
- [x] 医嘱执行确认 API（护士核对并确认执行医嘱） *（住院医嘱执行已实现）*
- [x] 交接班记录 API（交班内容、交接时间、交接人签名）
- [x] 病人过敏标识管理 API（新增/修改过敏标识，同步提醒医生）
- [x] 结构化过敏史与处方开立联动校验（兼容中英文分隔符）
- [x] 候诊区巡视记录 API（特殊病人状态记录、急诊绿色通道标记）

### 2.7 药房端扩展 API（库存精细化管理）

> **场景**：药师日常盘点库存、处理报损、监控效期、点评处方。

- [x] 库存盘点 API（创建盘点单/录入实盘数/生成盘盈盘亏）
- [x] 药品报损/报溢 API（报损原因、数量、审批流程）
- [x] 药品有效期预警 API（按阈值查询即将过期药品）
- [x] 处方开立/发药效期校验（过期药品禁止进入用药流程）
- [x] 停用药品处方拦截（药品状态与处方流程联动）
- [x] 处方点评 API（药师对处方合理性进行评价打分）
- [x] 发药统计 API（按日/按药品统计发药量）
- [x] 特殊药品管理 API（毒麻精放药品的专册登记、双人双锁）

### 2.8 收费端扩展 API（窗口业务）

> **场景**：收费员为不会自助操作的病人提供窗口挂号、收费、日结服务。

- [x] 窗口挂号 API（收费员代病人现场挂号，支持身份证读取）
- [x] 窗口挂号返回当天门诊挂号序号，成功页展示序号
- [x] 患者端挂号校验排班一致性并使用条件扣减号源
- [x] 挂号记录保存来源排班并精确返还退号号源
- [x] 患者端预约校验科室与排班一致性
- [x] 预约记录保存来源排班并精确返还取消预约号源
- [x] 窗口取消预约优先返还来源排班号源
- [x] 窗口退号 API（收费员处理现场退号）
- [x] 日结对账 API（个人日结汇总/明细、财务对账）
- [x] 预交金管理 API（充值/查询/退费/冲抵）
- [x] 预交金金额按 Decimal 定点精度计算并记录流水
- [x] 预交金退款接口（余额校验、退款流水和原因记录）
- [x] 收费退款同步支付流水状态（已退款）
- [x] 收费项目维护 API（新增/修改收费项目及价格）

### 2.9 检验科扩展 API（样本全流程）

> **场景**：检验科接收样本、跟踪流转、录入结果、报告危急值。

- [x] 样本接收/拒收 API（扫码接收、拒收原因记录）
- [x] 样本流转跟踪 API（样本在各环节的流转状态）
- [x] 危急值报告 API（自动识别危急值并通知临床医生）
- [x] 检验套餐维护 API（新增/修改检验套餐及项目组合）
- [x] 质控管理 API（质控品数据录入、质控图生成）

### 2.10 科室主任端 API（科室管理）

> **场景**：主任监管本科室运营，审批医生申请，查看科室统计数据。

- [x] 科室工作量统计 API（接诊人数、处方数、检查申请数、收入）
- [x] 科室收入统计 API（按日期统计科室已收处方收入）
- [x] 科室满意度查看 API（查看病人对本科室医生的评价及平均分）
- [x] 排班调休审批 API（审批医生的停诊/调班/加号申请）
- [x] 转诊/会诊审批 API（主任按本科室范围审批转诊/会诊申请，管理员可全量审批）

### 2.11 导医/挂号员 API（导诊分流）

> **场景**：导医引导病人就诊，挂号员为病人提供窗口服务。

- [x] 智能导诊 API（症状描述→推荐科室/医生）
- [x] 绿色通道标记 API（急诊/老年/孕妇/残障等优先标识）
- [x] 科室导航/FAQ API（科室位置、医生专长、常见问题）
- [x] 窗口预约确认/取消 API（挂号员帮助病人确认或取消预约）

### 2.12 管理员端扩展 API（系统治理）

> **场景**：管理员进行权限分配、数据维护、系统监控。

- [x] 用户权限管理 API（角色分配、菜单权限、数据权限）
 - [x] 数据导入导出 API（批量导入医生/病人/药品 Excel，含模板和全量校验）
- [x] 系统监控 API（在线人数、接口性能、错误率）

### 2.13 住院管理 API ✅

> **场景**：住院从入院到出院的完整业务流程。

- [x] 病区床位管理 API（病区/床位/床位图）
- [x] 入院登记 API（登记+押金缴纳）
- [x] 住院医嘱 API（长期/临时医嘱+执行记录）
- [x] 护士工作站 API（医嘱执行/护理记录/体温单）
- [x] 住院费用 API（一日清单/费用录入）
- [x] 出院结算 API（出院结算+清单打印）

### 2.14 手术麻醉 API ✅

- [x] 手术申请 API（申请单填写/审批）
- [x] 手术排程 API（手术室/台次安排）
- [x] 麻醉记录 API（术中麻醉单）

### 2.15 结构化电子病历 API ✅

- [x] 病历模板 API
- [x] 结构化病历 API（入院记录/首次病程等）
- [x] 病程记录 API
- [x] 查房记录 API
- [x] 病历质控 API

### 2.16 体检系统 API ✅

- [x] 体检套餐 API
- [x] 体检项目 API
- [x] 体检预约 API
- [x] 体检记录 API
- [x] 体检结果 API
- [x] 体检报告 API

### 2.17 急诊管理 API ✅

- [x] 急诊分诊 API（分级分诊 1~4 级）
- [x] 抢救记录 API（抢救时间轴/用药/操作）
- [x] 留观管理 API（留观登记/医嘱/费用）
- [x] 绿色通道 API（先救治后付费流程）
- [x] 急诊病历 API（急诊专用病历模板）

### 2.18 病案管理 API ✅

- [x] 病案首页 API（出院病案首页填写）
- [x] ICD-10 编码 API（诊断/手术编码查询）
- [x] 病案归档 API（归档/借阅/封存）
- [x] 病案质控 API（病案首页质控）

### 2.19 护理文书 API ✅

- [x] 入院护理评估 API（ADL/压疮/跌倒风险评估）
- [x] 护理计划 API（护理诊断+措施+目标）
- [x] 交接班记录 API（书面交接班报告）
- [x] 危重护理记录 API（ICU级护理记录）
- [x] 手术护理记录 API（术前/术中/术后护理）

### 2.20 门诊输液 API ✅

- [x] 输液医嘱 API
- [x] 配药核对 API（药房配药+护士核对）
- [x] 输液执行 API（穿刺/滴速/巡视）
- [x] 皮试管理 API
- [x] 输液结束 API（拔针/不良反应记录）

### 2.21 PACS/RIS API ✅

- [x] 检查申请 API（放射/超声/内镜申请）
- [x] 影像报告书写 API（诊断报告模板+书写）
- [x] 影像调阅 API（支持配置外部 DICOM Viewer 地址）
- [x] 报告审核 API（初诊/审核双签名）
- [x] 胶片管理 API（打印/云胶片）

### 2.22 抗菌药物管理 API ✅

- [x] 分级目录 API（非限制/限制/特殊级）
- [x] 越级审批 API
- [x] DDDs 统计 API（按药品统计使用数量、处方数和患者数）
- [x] 送检率统计 API
- [x] 围术期预防用药 API

### 2.23 院感管理 API ✅

- [x] 院感监测 API（感染病例监测）
- [x] 暴发预警 API（同种同源预警）
- [x] 消毒监测 API（环境监测记录）
- [x] 职业暴露 API（针刺伤等上报）
- [x] 院感上报 API（报表生成）

### 2.24 物资设备管理 API ✅

- [x] 设备台账 API（设备档案/卡片）
- [x] 设备维修 API（报修/维修记录）
- [x] 设备保养 API（定期保养计划）
- [x] 高值耗材追溯 API（一物一码追溯）
- [x] 资产盘点 API

### 2.25 血库管理 API ✅

- [x] 用血申请 API
- [x] 血型复核 API
- [x] 交叉配血 API
- [x] 发血管理 API
- [x] 输血反应上报 API

### 2.26 医保管理 API ✅

- [x] 医保目录对照 API
- [x] 医保结算 API（本地结算记录/对账）
- [x] 慢病管理 API（慢特病登记/限额）
- [x] DRG/DIP API（分组盈亏分析）
- [x] 医保控费 API（费用预警/超标提醒）

---

## 三、系统支撑现状与外部待办

- [x] 操作日志审计模块
- [x] 数据字典管理模块
- [x] 系统参数配置模块
- [x] 数据备份与恢复机制
- [x] 库存预警机制（低于阈值/过期提醒）
- [x] 违约处理机制（三次未取号暂停预约）
- [x] 短信通知接口（预约提醒/就诊提醒）
- [x] 支付接口（微信/支付宝）
- [x] 支付平台回调安全桥接（密钥鉴权、金额校验、幂等结算）
- [ ] 医保结算接口（已完成安全回调桥接；医保平台协议、网络白名单和联调待配置）
- [ ] LIS/PACS 系统对接（已完成安全入站桥接；厂商协议、网络白名单和联调待配置）
- [x] 消息通知中心（站内消息/短信/微信推送）
- [ ] 就诊导航/院内地图服务（本地院内路线图已实现；实时地图厂商服务待集成）
- [x] 电子签名服务（处方、病历、检验报告医生签名）
- [x] 文件存储服务（头像上传、检查报告附件、电子票据）
- [x] 定时任务调度器（库存预警检查、违约记录统计、数据备份状态；管理员可查看并手动触发）

---

## 四、Vue 3 前端页面建设记录

### 4.1 管理员端

- [x] 院区管理页面（院区 CRUD、科室归属与院区筛选）
- [x] 医生信息修改/删除页面
- [x] 病人信息修改页面
- [x] 科室信息修改/删除页面
- [x] 通知公告修改/删除页面
- [x] 药品信息修改/删除页面
- [x] 收费记录查看页面（已部分实现，需完善退费）
- [x] 报表统计页面（门诊量/财务/药品/工作量可视化图表）
- [x] 权限管理页面（角色分配、菜单权限、数据权限配置）
 - [x] 数据导入导出页面（批量导入医生/病人/药品 Excel）
- [x] 系统监控仪表盘（在线人数、接口性能、错误日志）
- [x] 系统参数配置页面
- [x] 数据字典管理页面
- [x] 操作日志审计页面（筛选/导出）

### 4.2 病人端

- [x] 病历查询页面
- [x] 检查检验结果查询页面
- [x] 健康档案页面
- [x] 就诊评价页面
- [x] 复诊预约页面
- [x] 消息通知中心页面（预约提醒、报告完成、就诊通知）
- [x] 排队进度查询页面（实时显示前面还有几人）
- [x] 家庭成员管理页面（为家人挂号、查看家人就诊记录）
- [x] 电子票据页面（发票列表、PDF 下载）
- [x] 就诊导航页面（科室位置、院内最短路线指引；实时地图服务待集成）

### 4.3 医生端

- [x] 病历书写/查询页面
- [x] 检查检验申请页面
- [x] 处方修改/取消页面
- [x] 排班查看优化（时段显示为周几上午/下午）
- [x] 转诊/会诊申请页面（发起转诊/会诊，查看审批状态）
- [x] 医嘱管理页面（住院医嘱已实现，门诊医嘱待补） *（住院已实现）*
- [x] 处方模板管理页面（创建/编辑/应用常用处方模板）
- [x] 诊断模板/ICD 编码页面（常用诊断、ICD-10 编码搜索）
- [x] 今日候诊列表页面（本医生今日候诊病人，含预检信息）
- [x] 病人检验结果查看页面（查看自己病人的检验报告）
- [x] 停诊/加号申请页面（向主任申请调休或额外加号）

### 4.4 药房端

- [x] 处方审核页面
- [x] 发药确认页面
- [x] 退药处理页面
- [x] 库存盘点页面（创建盘点单、录入实盘、生成盈亏）
- [x] 药品报损/报溢页面
- [x] 处方点评页面（对处方合理性评价打分）
- [x] 发药统计页面（按药师/按日/按药品统计）
- [x] 特殊药品页面（专册登记、双人复核）
- [x] 药品有效期预警页面（即将过期药品列表）

### 4.5 护士端

- [x] 候诊队列管理页面
- [x] 叫号显示页面
- [x] 生命体征录入页面

### 4.6 收费端

- [x] 窗口收费页面
- [x] 退费处理页面
- [x] 发票打印页面
- [x] 窗口挂号页面（为不会自助挂号的病人代挂号）
- [x] 日结对账页面（个人日结汇总、财务对账）
- [x] 预交金管理页面（充值、查询、退费）
- [x] 收费项目维护页面（项目编码、价格、启停状态）

### 4.7 护士端扩展页面

> **角色**：护士。完成预检后，执行输液/注射/皮试，记录护理文书。

- [x] 输液管理页面（开具/执行/巡视/结束）
- [x] 注射/皮试管理页面
- [x] 护理记录页面（病情观察、护理措施、效果评价） *（住院护理记录已实现）*
- [x] 医嘱执行确认页面（核对并执行医生医嘱） *（住院已实现）*
- [x] 交接班记录页面

### 4.8 检验科端页面

> **角色**：检验科技师。接收样本、跟踪流转、录入结果、质控管理。

- [x] 样本接收页面（扫码接收、拒收登记）
- [x] 样本流转跟踪页面
- [x] 危急值报告页面（自动识别危急值并通知临床）
- [x] 检验套餐维护页面
- [x] 质控管理页面（质控品数据录入、质控图）

### 4.9 科室主任端页面

> **角色**：科室主任。监管本科室运营，审批医生申请。

- [x] 科室统计仪表盘（工作量、收入、满意度概览）
- [x] 排班调休审批页面（审批医生的停诊/调班/加号申请）
- [x] 转诊/会诊审批页面

### 4.10 导医/挂号员端页面

> **角色**：导医、挂号员。引导病人、提供窗口服务。

- [x] 智能导诊页面（症状描述→推荐科室/医生）
- [x] 绿色通道管理页面（急诊/老年/孕妇优先标识及先救治后付费审批）
- [x] 窗口预约确认/取消页面
- [x] 就诊卡办理页面

### 4.11 住院管理端页面 ✅

- [x] 病区床位管理页面
- [x] 入院登记页面
- [x] 住院医嘱页面
- [x] 护士工作站页面（医嘱执行/护理记录）
- [x] 入院护理评估页面（ADL/压疮/跌倒/营养风险）
- [x] 护理计划页面（护理诊断/目标/措施/完成）
- [x] 危重护理记录页面（意识/GCS/氧疗/生命体征/尿量）
- [x] 手术护理记录页面（术前/术中/术后核查）
- [x] 住院费用页面
- [x] 出院结算页面
- [x] 电子病历页面（住院）

### 4.12 手术麻醉端页面 ✅

- [x] 手术管理页面（申请/排程/麻醉记录）

### 4.13 体检管理端页面 ✅

- [x] 体检综合管理页面（套餐/项目/预约/记录/结果）

### 4.14 急诊管理端页面 ✅

- [x] 急诊分诊页面
- [x] 抢救记录页面
- [x] 留观管理页面
- [x] 急诊病历页面

### 4.15 病案管理端页面 ✅

- [x] 病案首页填写页面
- [x] 病案归档/借阅页面
- [x] ICD-10 编码搜索页面
- [x] 病案首页质控页面

### 4.16 门诊输液端页面 ✅

- [x] 输液医嘱页面
- [x] 配药核对页面
- [x] 输液执行/巡视页面

### 4.17 PACS/RIS 端页面 ✅

- [x] 检查申请页面（医生）
- [x] 影像报告书写页面（放射科）
- [x] 影像调阅页面（DICOM Viewer 地址配置/调阅入口）

### 4.18 抗菌药物管理页面 ✅

- [x] 抗菌药物分级目录页面
- [x] 越级审批页面
- [x] 使用强度统计页面

### 4.19 院感管理页面 ✅

- [x] 院感监测页面
- [x] 暴发预警页面
- [x] 院感上报页面

### 4.20 物资设备管理页面 ✅

- [x] 设备台账页面
- [x] 设备维修/保养页面
- [x] 高值耗材追溯页面

### 4.21 血库管理页面 ✅

- [x] 用血申请页面
- [x] 配血/发血页面
- [x] 输血反应上报页面

### 4.22 医保管理页面 ✅

- [x] 医保目录对照页面
- [x] 医保结算页面
- [x] DRG/DIP 分析页面

---

## 五、数据库表建设记录

- [x] hoimsystem_queue（候诊队列）
- [x] hoimsystem_vital_sign（生命体征）
- [x] hoimsystem_lab_order（检查申请单）
- [x] hoimsystem_lab_result（检查结果）
- [x] hoimsystem_invoice（发票记录）
- [x] hoimsystem_follow_up（随访计划）
- [x] hoimsystem_review（满意度评价）
- [x] hoimsystem_operation_log（操作日志）
- [x] hoimsystem_dict（数据字典）
- [x] hoimsystem_config（系统参数）

---

## 六、新增数据库表（角色流程扩展）

### 6.1 病人就诊闭环

- [x] hoimsystem_message（消息通知中心：收件人、标题、内容、类型、已读状态、发送时间）
- [x] hoimsystem_family_member（家庭成员：病人 ID、成员姓名、关系、身份证号、手机号）
- [x] hoimsystem_patient_queue_status（复用候诊队列并提供实时进度计算接口）

### 6.2 医生诊疗深度

- [x] hoimsystem_referral（转诊记录）
- [x] hoimsystem_consultation（会诊记录）
- [x] hoimsystem_medical_order（住院医嘱：inpatient_order）
- [x] hoimsystem_prescription_template（处方模板）
- [x] hoimsystem_diagnosis_template（诊断模板：ICD-10 编码）

### 6.3 护士护理闭环

- [x] hoimsystem_infusion（输液记录）
- [x] hoimsystem_injection（注射记录）
- [x] hoimsystem_skin_test（皮试记录）
- [x] hoimsystem_nursing_record（护理记录） *（住院）*
- [x] hoimsystem_shift_handover（交接班记录）

### 6.4 药房库存精细管理

- [x] hoimsystem_stock_check（库存盘点单）
- [x] hoimsystem_stock_check_item（盘点明细）
- [x] hoimsystem_drug_damage（药品报损记录）
- [x] hoimsystem_special_drug（特殊药品管理：毒麻精放）

### 6.5 收费窗口业务

- [x] hoimsystem_daily_settlement（收费员日结）
- [x] hoimsystem_prepayment（预交金记录）

### 6.6 检验科样本管理

- [x] hoimsystem_lab_package（检验套餐：项目组合、价格、启停）
- [x] hoimsystem_sample（样本记录）
- [x] hoimsystem_sample_tracking（样本流转记录）
- [x] hoimsystem_qc_record（质控记录）

### 6.7 系统权限支撑

- [x] hoimsystem_user_role_permission（用户角色权限关联）
- [x] hoimsystem_menu_permission（菜单权限）

### 6.8 住院管理（已建） ✅

- [x] hoimsystem_ward（病区）
- [x] hoimsystem_bed（床位）
- [x] hoimsystem_admission（入院登记）
- [x] hoimsystem_inpatient_order（住院医嘱）
- [x] hoimsystem_inpatient_order_item（医嘱明细）
- [x] hoimsystem_order_execution（医嘱执行）
- [x] hoimsystem_nursing_record（护理记录）
- [x] hoimsystem_nursing_assessment（入院护理评估：ADL、压疮、跌倒、意识、营养风险）
- [x] hoimsystem_temperature_record（体温单）
- [x] hoimsystem_inpatient_charge（住院费用）
- [x] hoimsystem_discharge_summary（出院小结）

### 6.9 结构化EMR（已建） ✅

- [x] hoimsystem_medical_record_template（病历模板）
- [x] hoimsystem_structured_medical_record（结构化病历）
- [x] hoimsystem_progress_note（病程记录）
- [x] hoimsystem_ward_round（查房记录）
- [x] hoimsystem_medical_record_quality（病历质控）

### 6.10 手术麻醉（已建） ✅

- [x] hoimsystem_surgery_application（手术申请）
- [x] hoimsystem_surgery_schedule（手术排程）
- [x] hoimsystem_anesthesia_record（麻醉记录）

### 6.11 体检系统（已建） ✅

- [x] hoimsystem_exam_package（体检套餐）
- [x] hoimsystem_exam_item（体检项目）
- [x] hoimsystem_exam_appointment（体检预约）
- [x] hoimsystem_exam_record（体检记录）
- [x] hoimsystem_exam_result（体检结果）

### 6.12 急诊管理 ✅

- [x] hoimsystem_emergency_triage（急诊分诊：分级、症状、生命体征、分诊护士）
- [x] hoimsystem_emergency_rescue（抢救记录：抢救时间、用药、操作、参与人员）
- [x] hoimsystem_emergency_observation（留观记录：留观开始、结束时间、病情）
- [x] hoimsystem_emergency_green_channel（绿色通道：申请、审批、关闭、先救治后付费）
- [x] hoimsystem_emergency_medical_record（急诊病历：草稿、修改、签名）

### 6.13 病案管理 ✅

- [x] hoimsystem_medical_record_home（病案首页：基本信息、诊断、手术、费用汇总）
- [x] hoimsystem_icd10_diagnosis（ICD-10诊断编码库）
- [x] hoimsystem_icd10_operation（ICD-9-CM-3手术操作编码库）
- [x] hoimsystem_medical_record_archive（病案归档：归档号、位置、状态、借阅记录）
- [x] hoimsystem_medical_record_home_quality（病案首页质控：检查项、结果、问题、评分）

### 6.14 护理文书 ✅

- [x] hoimsystem_nursing_assessment（入院护理评估）
- [x] hoimsystem_nursing_plan（护理计划）
- [x] hoimsystem_critical_care_record（危重护理记录）
- [x] hoimsystem_surgery_nursing_record（手术护理记录：术前、术中、术后）

### 6.15 门诊输液 ✅

- [x] hoimsystem_infusion_order（输液医嘱）
- [x] hoimsystem_infusion_execution（输液执行/巡视记录，复用 infusion_observation）

### 6.16 PACS/RIS ✅

- [x] hoimsystem_imaging_order（影像检查申请）
- [x] hoimsystem_imaging_report（影像报告）
- [x] hoimsystem_imaging_template（影像报告模板）

### 6.17 抗菌药物管理 ✅

- [x] hoimsystem_antibiotic_grade（复用药品抗菌等级字段）
- [x] hoimsystem_antibiotic_approval（越级使用审批）

### 6.18 院感管理 ✅

- [x] hoimsystem_infection_case（院感病例）
- [x] hoimsystem_disinfection_monitor（消毒监测）
- [x] hoimsystem_occupational_exposure（职业暴露）

### 6.19 物资设备管理 ✅

- [x] hoimsystem_equipment（设备台账）
- [x] hoimsystem_equipment_maintenance（设备维修）
- [x] hoimsystem_equipment_inspection（设备保养）
- [x] hoimsystem_consumable_trace（高值耗材追溯）

### 6.20 血库管理 ✅

- [x] hoimsystem_blood_request（用血申请）
- [x] hoimsystem_blood_cross_match（交叉配血）
- [x] hoimsystem_blood_issue（发血记录）
- [x] hoimsystem_transfusion_reaction（输血反应）

### 6.21 医保管理 ✅

- [x] hoimsystem_insurance_catalog（医保目录对照）
- [x] hoimsystem_insurance_settlement（医保结算）
- [x] hoimsystem_drg_grouping（DRG分组结果）

---

## 七、现有表待补充字段

- [x] hoimsystem_patient：allergy_history（过敏史）
- [x] hoimsystem_prescription：status（处方状态）、create_time（创建时间）
- [x] hoimsystem_medical_record：registration_uuid（关联挂号/预约）
- [x] hoimsystem_pharmaceutical：status（状态：正常/停用）
- [x] hoimsystem_charge：status 扩展值（2=已退费）
- [x] hoimsystem_breach_record：breach_time（违约时间）、breach_type（违约类型）
- [x] hoimsystem_department：campus_id（所属院区）

---

## 八、技术优化

- [x] 密码加密存储（当前明文，需改为 bcrypt/argon2）
- [x] accessToken 使用 JWT 替代纯字符串
- [x] 接口权限校验细化（当前部分接口未校验角色）
- [x] 数据库事务处理完善（部分操作缺少 rollback）
- [x] 核心财务金额使用 DECIMAL(12,2) 定点精度（预交金、收费、支付、住院费用、医保结算）
- [x] 输入参数校验加强（Pydantic 模型完善）
- [x] 异常处理规范化（统一错误码和错误信息）
- [x] 分页查询支持（列表接口缺少分页）
- [x] 文件上传支持（头像上传、检查报告附件）
- [x] 定时任务（库存预警检查、违约记录统计）
- [x] 接口性能优化（核心分页、关联预加载、数据库聚合、组合索引、GZip、流式导出、单次响应序列化）
- [x] PostgreSQL 有界连接池和语句超时，生产迁移与 API/调度进程分离
- [x] Redis 跨实例临床事件广播与断线回放（鉴权 token 不进入 URL）
- [x] Prometheus 请求量/时延/在途指标，存活与数据库/迁移/Redis 就绪探针
- [x] PostgreSQL 隔离压测环境、分级数据量、运行时 token 和业务成功断言
- [x] 前端路由懒加载、Element Plus/ECharts 按需引入、bundle 预算

---

## 九、安全与合规

- [x] 密码传输加密（前端通过后端公钥加密，后端解密后仅以 bcrypt 存储；兼容旧客户端）
- [x] SQL 注入防护（ORM 参数绑定审计完成，恶意查询回归测试覆盖）
- [x] XSS 防护（Vue 默认转义，富文本预览增加白名单清理）
- [x] CSRF 防护（JWT 仅通过自定义请求头传输，并对状态变更请求校验 Origin 白名单）
- [x] 接口频率限制（登录接口失败次数限制，其他业务接口仍需按网关策略配置）
- [x] 敏感数据脱敏（按角色最小权限掩码身份证号、手机号，患者本人和必要业务角色保留完整值）
- [ ] 等保合规检查（已完成代码安全基线自评；第三方测评和生产运营证据待补）

---

## 十、文档维护

- [x] 需求文档（demandDoc.md）
- [x] API 文档（apiDoc.md）
- [x] 数据库文档（databaseDoc.md）
- [x] 部署文档（deployDoc.md，含开发/生产/Docker、HTTPS、Systemd和迁移说明）
- [x] 前端组件文档（architecture.md、coding-standards.md）
- [x] 测试用例文档（testing.md、api-test-report.md、frontend-test-report.md）
- [x] 用户操作手册（user-manual.md，覆盖患者、医生、护士、药师、收费/挂号员和管理员）

---

## 十一、长期规划

- [x] 住院管理模块
- [x] 手术室管理模块
- [x] 体检中心模块
- [x] 多院区/多科室管理（院区主数据、科室归属、管理端与患者导航筛选）
- [ ] 移动端 App/小程序（已提供可安装移动 Web/PWA 基础；原生 App/小程序待立项）
- [ ] AI 辅助诊断（已增强可解释规则导诊和急症提示；临床验证后的 AI 模型待立项）
- [x] 电子签名（处方、病历医生签名）

---

## 十二、未来开发优先级建议

### 第一阶段：生产验证与合规

1. 在目标硬件完成 PostgreSQL 容量基线、慢查询分析和连接池参数校准。
2. 完成等保第三方测评、备份恢复演练、告警通知和日志留存策略。
3. 与真实 LIS/PACS/医保/支付厂商完成字段、签名、幂等、乱序和对账联调。

### 第二阶段：临床深度

4. 接入经临床验证的专业 CDSS 知识库、检验趋势和肾功能剂量算法。
5. 跟随官方版本维护 CHS-DRG/DIP 规则包并建立规则发布审批。
6. 补齐原生 DICOM 存储/路由、检查工作列表和影像生命周期治理。

### 第三阶段：体验与规模化

7. 为剩余高增长业务列表补前端服务端分页、筛选和虚拟滚动。
8. 建设互联网医院、复诊配药和移动端/小程序（需独立合规立项）。
9. 基于真实运营数据完善容量趋势、数据归档和冷热分层。

---

*最后更新：2026-09-03*
