...
def issue_blood(req: dict, current_user: User = Depends(require_roles(*BLOOD_ROLES)), db: Session = Depends(get_db)):
    ...
    item = BloodIssue(request_id=request.request_id, unit_no=req.get("unit_no", "").strip(), component=request.component, volume=int(req.get("volume", request.volume)), issuer_id=current_user.user_id, issue_time=datetime.datetime.now())
    if not item.unit_no or item.volume <= 0:
        return {"code": 400, "msg": "血袋编号和发血量不能为空"}
    request.status = 3
    db.add(item)
    db.commit()
