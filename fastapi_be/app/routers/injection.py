...
def execute_injection(req: InjectionIdRequest, current_user: User = Depends(require_roles(*NURSING_ROLES)), db: Session = Depends(get_db)):
    updated = (
        db.query(InjectionOrder)
        .filter(InjectionOrder.injection_id == req.injection_id, InjectionOrder.status == 0)
        .update({InjectionOrder.status: 1, InjectionOrder.nurse_id: current_user.user_id, InjectionOrder.execute_time: datetime.datetime.now()}, synchronize_session=False)
    )
    if updated != 1:
...
def complete_injection(req: InjectionIdRequest, current_user: User = Depends(require_roles(*NURSING_ROLES)), db: Session = Depends(get_db)):
    updated = (
        db.query(InjectionOrder)
        .filter(InjectionOrder.injection_id == req.injection_id, InjectionOrder.status == 1, InjectionOrder.nurse_id == current_user.user_id)
        .update({InjectionOrder.status: 2, InjectionOrder.complete_time: datetime.datetime.now()}, synchronize_session=False)
    )
    if updated != 1:
...
def cancel_injection(req: InjectionIdRequest, current_user: User = Depends(require_roles(*CLINICAL_ROLES)), db: Session = Depends(get_db)):
    updated = (
        db.query(InjectionOrder)
        .filter(InjectionOrder.injection_id == req.injection_id, InjectionOrder.status == 0)
        .update({InjectionOrder.status: 3, InjectionOrder.complete_time: datetime.datetime.now()}, synchronize_session=False)
    )
    if updated != 1:
