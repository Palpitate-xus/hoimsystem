...
def list_insurance_settlement(current_user: User = Depends(require_roles(*MANAGE_ROLES)), db: Session = Depends(get_db)):
    ...
    return {"code": 200, "msg": "success", "data": [{"settlement_id": item.settlement_id, "patient_name": item.patient.name if item.patient else "", "insurance_no": item.insurance_no, "total_amount": item.total_amount, "covered_amount": item.covered_amount, "self_amount": item.self_amount, "status": item.status, "status_text": {0: "处理中", 1: "成功", 2: "失败"}.get(item.status, ""), "settlement_time": item.settlement_time.strftime("%Y-%m-%d %H:%M:%S")} for item in items]}
...
def create_insurance_settlement(req: dict, current_user: User = Depends(require_roles(*CASHIER_ROLES)), db: Session = Depends(get_db)):
    ...
    item = InsuranceSettlement(patient_id=patient.patient_id, insurance_no=req["insurance_no"], total_amount=total, covered_amount=covered, self_amount=round(total - covered, 2), status=1, operator_id=current_user.user_id, settlement_time=datetime.datetime.now())
    db.add(item); db.commit()
    return {"code": 200, "msg": "结算成功", "data": {"settlement_id": item.settlement_id, "self_amount": item.self_amount}}
