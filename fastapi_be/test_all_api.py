...
# login patient and save JWT token
resp = test("login success", "POST", "/api/login", data={"username":"370101199001011234","password":"123456"})
if resp and resp.get("data") and resp["data"].get("accesstoken"):
    tokens["patient"] = resp["data"]["accesstoken"]

test("login fail", "POST", "/api/login", data={"username":"wrong","password":"wrong"}, expected_code=200, check_fn=lambda b: b.get("code") == 500)

test("userInfo", "POST", "/api/userInfo", data={"accesstoken": get_token("patient")})
...
# create notice
test("create notice", "POST", "/api/notice/create", headers={"accesstoken": get_token("patient")}, data={"title":"通知1","content":"内容","isemergency":1,"towho":["医生","病人"],"expiredtime":"2026-12-31"})
test("get notice list", "GET", "/api/notice/getList", headers={"accesstoken": get_token("patient")})
...
# login as doctor and save JWT token
resp = test("login doctor", "POST", "/api/login", data={"username":"doc01","password":"123456"})
if resp and resp.get("data") and resp["data"].get("accesstoken"):
    tokens["doctor"] = resp["data"]["accesstoken"]
...
test("doctor schedule register (today)", "POST", "/api/doctorScheduleManagement/register", headers={"accesstoken": get_token("doctor")}, data={"schedule":[f"{today_cn}上午", f"{today_cn}下午"],"specialist":1,"number":20,"doctor":1})
test("doctor schedule getList", "GET", "/api/doctorScheduleManagement/getList", headers={"accesstoken": get_token("doctor")})
...
test("prescription create", "POST", "/api/prescriptionManagement/create", headers={"accesstoken": get_token("doctor")}, data={"patient":2,"phas":[{"id":1,"number":2}]})

# get prescription list (doctor view)
resp = test("prescription getList (doctor)", "GET", "/api/prescriptionManagement/getList", headers={"accesstoken": get_token("doctor")})

# get prescription list (patient view)
test("prescription getList (patient)", "GET", "/api/prescriptionManagement/getList", headers={"accesstoken": get_token("patient")})
...
resp = test("appointment create", "POST", "/api/appointmentManagement/create", headers={"accesstoken": get_token("patient")}, data={"id":1,"date":"2026-05-01","department_id":1,"doctor_id":1,"time":"上午","specialist":1})

# get appointment list
resp = test("appointment getList", "GET", "/api/appointmentManagement/getList", headers={"accesstoken": get_token("patient")})
...
if appointment_uuid:
    ...
    test("appointment getList after cancel", "GET", "/api/appointmentManagement/getList", headers={"accesstoken": get_token("patient")}, check_fn=lambda b: b.get("data") and b["data"][0].get("status") == "已取消")

# registration - need schedule for today to show in registrationList
test("registration list (today)", "GET", "/api/registrationManagement/registrationList", headers={"accesstoken": get_token("patient")})

# create registration
resp = test("registration create", "POST", "/api/registrationManagement/create", headers={"accesstoken": get_token("patient")}, data={"id":1,"doctor_id":1,"department_id":1,"specialist":1})

# get registration list
resp = test("registration getList", "GET", "/api/registrationManagement/getList", headers={"accesstoken": get_token("patient")})
if resp and resp.get("data"):
...
if registration_uuid:
    ...
    test("registration getList after cancel", "GET", "/api/registrationManagement/getList", headers={"accesstoken": get_token("patient")}, check_fn=lambda b: b.get("data") and b["data"][0].get("status") == "已取消")

# charge
resp = test("charge getList (patient)", "GET", "/api/chargeManagement/getList", headers={"accesstoken": get_token("patient")})
if resp and resp.get("data"):
    charge_id = resp["data"][0]["id"]

if charge_id:
    test("charge commit", "POST", "/api/chargeManagement/charge", data={"id": charge_id})
    test("charge getList after commit", "GET", "/api/chargeManagement/getList", headers={"accesstoken": get_token("patient")}, check_fn=lambda b: b.get("data") and b["data"][0].get("status") == 1)

# charge getList as admin
test("charge getList (admin)", "GET", "/api/chargeManagement/getList", headers={"accesstoken": get_token("patient")})
